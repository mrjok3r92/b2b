...
if ($order['status'] !== 'pending') {
    setFlashMessage('error', 'Această comandă nu este în așteptare și nu poate fi respinsă.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
        ...
        // Obține motivul respingerii
        $reason = isset($_POST['reason']) ? sanitizeInput($_POST['reason']) : '';
        
        // Validare motiv
        if (empty($reason)) {
            $error = 'Vă rugăm să furnizați un motiv pentru respingerea comenzii.';
        } else {
            // Respinge comanda
            $result = $orderObj->rejectOrder($order_id, $_SESSION['user_id'], $reason);
            
            if ($result) {
                // Creare notificare
                $notificationObj->createOrderRejectedNotification(
                    $order_id, 
                    $order['order_number'], 
                    $order['client_id'], 
                    $order['user_id'],
                ...
                setFlashMessage('success', 'Comanda a fost respinsă cu succes.');
                redirect('view.php?id=' . $order_id);
            } else {
                $error = 'A apărut o eroare la respingerea comenzii. Vă rugăm să încercați din nou.';
            }
        }
    }
}
...
// Titlu pagină
$pageTitle = 'Respingere Comandă #' . $order['order_number'] . ' - Panou de Administrare';
...
<div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <h1 class="text-xl font-bold text-gray-900">Respingere comandă #<?php echo $order['order_number']; ?></h1>
    </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-red-700">
                            <strong>Atenție!</strong> Sunteți pe cale să respingeți comanda <strong>#<?php echo $order['order_number']; ?></strong> 
                            în valoare de <strong><?php echo formatAmount($order['total_amount']); ?> Lei</strong> pentru clientul 
                            <strong><?php echo htmlspecialchars($order['company_name']); ?></strong>.
                        </p>
                        <p class="text-sm text-red-700 mt-2">
                            Această acțiune nu poate fi anulată. Clientul va fi notificat despre respingerea comenzii.
                        </p>
                    </div>
                </div>
            </div>
            
            <form method="POST" action="reject.php?id=<?php echo $order_id; ?>">
                <!-- CSRF token -->
                <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                
                <!-- Motiv respingere -->
                <div class="mb-4">
                    <label for="reason" class="block text-sm font-medium text-gray-700 mb-1">Motiv respingere <span class="text-red-500">*</span></label>
                    <textarea id="reason" name="reason" rows="4" required
                              ...
                              placeholder="Vă rugăm să furnizați motivul pentru care această comandă este respinsă..."><?php echo htmlspecialchars($reason); ?></textarea>
                    <p class="mt-1 text-xs text-gray-500">Acest motiv va fi vizibil pentru client în detaliile comenzii și în notificare.</p>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                        <i class="fas fa-times mr-1"></i> Respinge comanda
                    </button>
                </div>
            </form>
        </div>
